...
class MathEngine {
  constructor(arr) {
    ...
    if (!arr.every((value) => typeof value === "number" && !isNaN(value))) throw new Error(INVALID_NBR_ARRAY_MSG)
  ...
  Mean = (precision = 8) => {
    ...
    return +(this._Array.reduce((prev, curr) => prev + curr, 0) / this.Length).toPrecision(precision)
  }
  ...
  StdDeviation = (precision = 8) => {
    ...
    //    SQRT(0.5) = 0.70710678
    return +Math.sqrt(
      this._Array.map((value) => Math.pow(value - this.Mean(), 2)).reduce((prev, curr) => prev + curr) /
        (this.Length - 1),
